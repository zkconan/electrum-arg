...
;Variables
  ...
  !define PRODUCT_NAME "Electrum (Argentum)"
  !define PRODUCT_WEB_SITE "https://github.com/argentumproject/electrum-arg"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-arg-setup.exe"
...
;Interface Settings
  ...
  !define MUI_ICON "tmp\electrum-arg\icons\electrum.ico"
...
Section
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-arg-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-arg-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-arg-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-arg-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-arg-${PRODUCT_VERSION}.exe" 0

  ;Links argentum: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\argentum" "" "URL:argentum Protocol"
  WriteRegStr HKCU "Software\Classes\argentum" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\argentum" "DefaultIcon" "$\"$INSTDIR\electrum.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\argentum\shell\open\command" "" "$\"$INSTDIR\electrum-arg-${PRODUCT_VERSION}.exe$\" $\"%1$\""
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\argentum"
